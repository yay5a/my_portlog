...
| **Eero LAN ↔ GS108E ↔ SE3008v2 ↔ WAP #2** | **Yes** | This branch only |
| **Eero LAN ↔ SE3005 ↔ WAP #1** | **No** | Bypasses GS108E |
| **Eero LAN ↔ Desktop (direct)** | **No** | Bypasses GS108E |
| **LAN broadcasts on GS108E branch** (mDNS/SSDP/ARP) | **Yes** | Local segment chatter |
...
- Python utilities – triage_parser.py, ip_extractor.c for per-host byte counts  
- Wireshark – visual inspection of flow distribution  
- nmap / arp-scan – endpoint enumeration  
- AI-assisted parsing scripts & network configuration
...
- **Excluded the WAN** path entirely (ONT ↔ Eero via TL‑SX1008).
- **Excluded** several LAN nodes (Eero Wi‑Fi clients; SE3005/WAP #1; direct Eero↔Desktop).
...
## Why the 50 TB Anomaly Cannot Be Attributed From These Pcaps
...
Operational notes:
- Confirm that all devices used for network monitoring can support **≥5 Gbps**; such as,
    - Use a multi‑gig managed switches and capture NIC.
...
## Appendix — Evidence
A1. ISP portal screenshots (Jun–Sep anomaly window; **Oct shown only as post-support baseline**)
![screenshot of montly total data usage](/strange_.png)

---
A2. Summary of data parsed from pcaps (Traffic intermittently captured beginning Aug. 28th through Sept. 19th, with & without capture filters)
| Metric        | Value                             |
| ------------- | --------------------------------- |
